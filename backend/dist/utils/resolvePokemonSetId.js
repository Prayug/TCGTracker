...
exports.foldSetToken = foldSetToken;
exports.foldSetTokenCompact = foldSetTokenCompact;
exports.resolveCatalogSetId = resolveCatalogSetId;
exports.resolvePokemonApiSetId = resolvePokemonApiSetId;
...
function foldSetTokenCompact(value) {
    return foldSetToken(value).replace(/and/g, '');
}
function catalogIdLookup(catalogSetIds) {
    const map = new Map();
...
function resolvePromoSetId(slug, nameFold, knownIds) {
 ...
 */
function resolveCatalogSetId(setId, setName, apiSets, catalogSetIds = []) {
    if (!setId)
        return null;
...
function resolvePokemonApiSetId(setId, setName, apiSets) {
    if (!setId || apiSets.length === 0)
        return null;
    const knownIds = new Set(apiSets.map((set) => set.id));
    const exactFold = new Map();
    const suffixNames = [];
    const addExact = (key, id) => {
        if (!key || GENERIC_FOLD_NAMES.has(key))
            return;
        if (!exactFold.has(key))
            exactFold.set(key, id);
    };
