...
const packPoolDedupe_1 = require("../packPoolDedupe");
describe('packCardIdentity', () => {
    it('treats Pokemon API and TCGCSV ids as the same POP Series 3 Blastoise', () => {
        ...
        expect((0, packPoolDedupe_1.packCardIdentity)(api)).toBe((0, packPoolDedupe_1.packCardIdentity)(tcgcsv));
        expect((0, packPoolDedupe_1.packCardIdentity)(api)).toBe('popseries3|1|blastoise');
    });
    it('strips leading zeros on card numbers', () => {
        expect((0, packPoolDedupe_1.packCardIdentity)({
            name: 'Pikachu',
            number: '025',
            set: { name: 'Base Set' },
        })).toBe((0, packPoolDedupe_1.packCardIdentity)({
            name: 'Pikachu',
            number: '25',
            set: { name: 'Base Set' },
        }));
...
describe('dedupePackPoolCards', () => {
    it('keeps the Pokemon API id and merges PSA 10 price from the TCGCSV row', () => {
        const deduped = (0, packPoolDedupe_1.dedupePackPoolCards)([
            {
                id: 'tcgcsv-83891',
                name: 'Blastoise',
                number: '1',
    ...
    it('prefers a non-tcgcsv id when both have images', () => {
        const winner = (0, packPoolDedupe_1.preferPackPoolCard)({ id: 'tcgcsv-1', images: { small: 'a' }, marketPrice: 9 }, { id: 'base1-4', images: { small: 'b' }, marketPrice: 1 });
