...
const resolvePokemonSetId_1 = require("../resolvePokemonSetId");
const SETS = [
...
const resolve = (id, name) => (0, resolvePokemonSetId_1.resolvePokemonApiSetId)(id, name, SETS);
describe('foldSetToken', () => {
    it('folds diacritics and ampersands', () => {
        expect((0, resolvePokemonSetId_1.foldSetToken)('Pokémon GO')).toBe('pokemongo');
        expect((0, resolvePokemonSetId_1.foldSetToken)('Black & White')).toBe('blackandwhite');
    });
});
describe('resolvePokemonApiSetId', () => {
    ...
    it('maps promo slugs to Black Star / McDonalds sets', () => {
        ...
        expect(resolve('mcdonalds25thanniversarypromos', "McDonald's 25th Anniversary Promos")).toBe('mcd21');
    });
...
describe('resolveCatalogSetId', () => {
    it('prefers native catalog ids (Japanese set codes)', () => {
        expect((0, resolvePokemonSetId_1.resolveCatalogSetId)('S12a', 'VSTARユニバース', SETS, ['S12a', 'M2a', 'MC'])).toBe('S12a');
        expect((0, resolvePokemonSetId_1.resolveCatalogSetId)('M2a', 'MEGAドリームex', SETS, ['S12a', 'M2a'])).toBe('M2a');
    });
    it('falls back to API ids when the slug is not in catalog', () => {
        expect((0, resolvePokemonSetId_1.resolveCatalogSetId)('pokemongo', 'Pokemon GO', SETS, ['S12a'])).toBe('pgo');
    });
});
