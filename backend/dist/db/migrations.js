...
exports.migrations = [
    ...
    },
    {
        ...
        up: async (db) => {
            ...
            const columnExists = async (table, column) => {
                const row = await get(`SELECT name FROM pragma_table_info('${table}') WHERE name = ?`, [column]);
                return !!row;
            };
            const indexExists = async (name) => {
                const row = await get(`SELECT name FROM sqlite_master WHERE type='index' AND name = ?`, [name]);
                return !!row;
            };
            ...
            if (!(await columnExists('card_predictions', 'signal_score'))) {
                ...
                logger_1.logger.info('Added signal_score to card_predictions');
            }
            // --- 2. Deduplicate calibration_samples then enforce unique index ---
            const calCount = await get('SELECT COUNT(*) AS n FROM calibration_samples');
            const calDistinct = await get(`SELECT COUNT(*) AS n FROM (
           SELECT 1 FROM calibration_samples
           GROUP BY horizon, card_id, source, COALESCE(prediction_date, '')
         )`);
            if (((_a = calCount === null || calCount === void 0 ? void 0 : calCount.n) !== null && _a !== void 0 ? _a : 0) > ((_b = calDistinct === null || calDistinct === void 0 ? void 0 : calDistinct.n) !== null && _b !== void 0 ? _b : 0)) {
                logger_1.logger.info('Deduplicating calibration_samples', {
                    before: calCount === null || calCount === void 0 ? void 0 : calCount.n,
                    distinct: calDistinct === null || calDistinct === void 0 ? void 0 : calDistinct.n,
                });
          ...
          GROUP BY horizon, card_id, source, COALESCE(prediction_date, '')`);
                ...
                await run('CREATE INDEX IF NOT EXISTS idx_calibration_samples_horizon ON calibration_samples(horizon)');
                await run('CREATE INDEX IF NOT EXISTS idx_calibration_samples_prediction_date ON calibration_samples(prediction_date)');
                const after = await get('SELECT COUNT(*) AS n FROM calibration_samples');
                logger_1.logger.info('calibration_samples deduped', { after: after === null || after === void 0 ? void 0 : after.n });
            }
            if (!(await indexExists('uq_calibration_samples'))) {
                await run('CREATE UNIQUE INDEX IF NOT EXISTS uq_calibration_samples ON calibration_samples(horizon, card_id, source, prediction_date)');
                logger_1.logger.info('Created uq_calibration_samples');
            }
        ...
        WHERE (variant_key IS NULL OR variant_key = '')
          AND unique_identifier IS NOT NULL AND unique_identifier <> ''`);
            ...
            await run(`UPDATE card_predictions SET unique_identifier = '' WHERE unique_identifier IS NULL`);
        ...
        WHERE id NOT IN (
          ...
          GROUP BY run_id, card_id, COALESCE(unique_identifier, '')
            ...
            if (!(await indexExists('uq_card_predictions_run_card_uid'))) {
                await run(`CREATE UNIQUE INDEX IF NOT EXISTS uq_card_predictions_run_card_uid
           ON card_predictions(run_id, card_id, unique_identifier)`);
            ...
            if (await columnExists('card_mappings', 'image_small')) {
                try {
                    ...
                    logger_1.logger.info('Dropped unused image_small/image_large columns');
                }
                catch (err) {
                    logger_1.logger.warn('Could not drop legacy image columns (SQLite version?)', { error: err });
                }
            }
        ...
        checked_at TEXT DEFAULT (datetime('now'))
            ...
            await run('CREATE INDEX IF NOT EXISTS idx_data_quality_checked ON data_quality_checks(checked_at)');
        ...
        FOREIGN KEY (collection_id) REFERENCES user_collections(id) ON DELETE SET NULL
            ...
            if (!(await columnExists('price_alerts', 'alert_type'))) {
                await run(`ALTER TABLE price_alerts ADD COLUMN alert_type TEXT NOT NULL DEFAULT 'price_threshold'`);
            }
            ...
            // --- 11. Helpful indexes ---
            await run('CREATE INDEX IF NOT EXISTS idx_prediction_results_status_pred ON prediction_results(status, prediction_id)');
            await run('CREATE INDEX IF NOT EXISTS idx_user_collections_user ON user_collections(user_id)');
            logger_1.logger.info('Migration 24: data integrity and platform features complete');
        },
        down: async (_db) => {
            logger_1.logger.info('Skipping migration 24 rollback (destructive / SQLite limitation)');
        },
    },
    {
        id: 25,
        ...
        up: async (db) => {
            ...
            await run('CREATE INDEX IF NOT EXISTS idx_users_verification_token ON users(email_verification_token)');
            logger_1.logger.info('Migration 25: email verification columns added');
        },
        down: async (_db) => {
            logger_1.logger.info('Skipping migration 25 rollback (SQLite limitation)');
        },
    },
    {
        id: 26,
        ...
        lastRefreshedAt INTEGER
            ...
            logger_1.logger.info('Migration 26: graded data verification columns + purge complete');
        },
        down: async (_db) => {
            logger_1.logger.info('Skipping migration 26 rollback (SQLite limitation)');
        },
    },
    {
        id: 27,
        ...
        learnedAt INTEGER NOT NULL
            ...
            logger_1.logger.info('Migration 27: pc_set_mappings table created');
        },
        down: async (_db) => {
            logger_1.logger.info('Skipping migration 27 rollback (SQLite limitation)');
        },
    },
    {
        id: 28,
        ...
        PRIMARY KEY (cardId, date, grader, grade)
            ...
            await run('CREATE INDEX IF NOT EXISTS idx_graded_price_history_card_date ON graded_price_history(cardId, date)');
            await run('CREATE INDEX IF NOT EXISTS idx_graded_price_history_lookup ON graded_price_history(cardId, grader, grade, date)');
        ...
        WHERE price IS NOT NULL AND price > 0`);
            logger_1.logger.info('Migration 28: graded_price_history table created + seeded');
        },
        down: async (db) => {
            const run = (sql) => new Promise((resolve, reject) => {
    ...
    },
    {
        ...
        PRIMARY KEY (cardId, date)
            ...
            await run('CREATE INDEX IF NOT EXISTS idx_population_history_date ON population_history(date)');
            await run('CREATE INDEX IF NOT EXISTS idx_population_history_psa10 ON population_history(cardId, psa10)');
        ...
        WHERE cardId IS NOT NULL AND cardId != ''
          AND json_extract(payload, '$.companies.psa.total') IS NOT NULL`);
            logger_1.logger.info('Migration 29: population_history created + seeded');
        },
        down: async (db) => {
            const run = (sql) => new Promise((resolve, reject) => {
                db.run(sql, (err) => (err ? reject(err) : resolve()));
    ...
    },
    {
      ...
      `);
            ...
            await run(`UPDATE graded_refresh_queue SET lastRefreshedAt = NULL WHERE cardId IN (SELECT cardId FROM _bad_slab_ids)`);
            await run(`DROP TABLE IF EXISTS _bad_slab_ids`);
            logger_1.logger.info('Migration 30: cleared unnumbered main-set SKUs mapped to subset/full-art slab products');
        },
        down: async () => {
    ...
    },
    {
        ...
        up: async (db) => {
            ...
            logger_1.logger.info('Migration 32: last-sold and listed-ask columns on graded_prices');
        },
        down: async () => {
            logger_1.logger.info('Skipping migration 32 rollback (SQLite limitation)');
        },
    },
    {
        id: 33,
        ...
        WHERE listedCount IS NULL`);
            logger_1.logger.info('Migration 33: listedCount history on graded_price_history');
        },
        down: async () => {
            logger_1.logger.info('Skipping migration 33 rollback (SQLite limitation)');
        },
    },
    {
        id: 34,
        ...
        up: async (db) => {
            ...
            await run('CREATE INDEX IF NOT EXISTS idx_gph_grader_grade_date ON graded_price_history(grader, grade, date)');
            logger_1.logger.info('Migration 34: graded_price_history (grader, grade, date) index');
        },
        down: async (db) => {
            await new Promise((resolve, reject) => {
                db.run('DROP INDEX IF EXISTS idx_gph_grader_grade_date', (err) => err ? reject(err) : resolve());
            });
        },
    },
    {
        ...
        up: async (db) => {
            ...
            await run('CREATE INDEX IF NOT EXISTS idx_catalog_cards_match_name ON catalog_cards(matchName)');
        ...
        SELECT ourSetName, 'en', consoleName, learnedAt FROM pc_set_mappings`);
           ...
           VALUES (?, 'ja', ?, ?)
           ON CONFLICT(ourSetName, language) DO UPDATE SET consoleName = excluded.consoleName`, [ourSetName, consoleName, Date.now()]);
            }
            logger_1.logger.info('Migration 35: language + matchName columns; JP pc_set_mappings seeds');
        },
        down: async () => {
            logger_1.logger.info('Skipping migration 35 rollback (SQLite limitation)');
        },
    },
    {
        id: 36,
      ...
      END`;
          ...
          UNIQUE(cardId, variantKey, grader, grade)
                ...
                await run(`INSERT OR IGNORE INTO graded_prices_v36 (
          ...
          ORDER BY fetchedAt DESC`);
                ...
                await run('CREATE INDEX IF NOT EXISTS idx_graded_prices_card ON graded_prices(cardId)');
                await run('CREATE INDEX IF NOT EXISTS idx_graded_prices_card_variant ON graded_prices(cardId, variantKey)');
                await run('CREATE INDEX IF NOT EXISTS idx_graded_prices_grader ON graded_prices(grader, grade)');
                await run('CREATE INDEX IF NOT EXISTS idx_graded_prices_verified ON graded_prices(verified)');
            }
          ...
          PRIMARY KEY (cardId, variantKey, date, grader, grade)
                ...
                await run(`INSERT OR IGNORE INTO graded_price_history_v36 (
          ...
          ORDER BY verified DESC, date DESC`);
                ...
                await run('CREATE INDEX IF NOT EXISTS idx_graded_price_history_card_date ON graded_price_history(cardId, date)');
                await run('CREATE INDEX IF NOT EXISTS idx_graded_price_history_lookup ON graded_price_history(cardId, variantKey, grader, grade, date)');
                await run('CREATE INDEX IF NOT EXISTS idx_gph_grader_grade_date ON graded_price_history(grader, grade, date)');
            }
            ...
            const queueExists = await new Promise((resolve, reject) => {
                db.get(`SELECT name FROM sqlite_master WHERE type='table' AND name=?`, ['graded_refresh_queue'], (err, row) => (err ? reject(err) : resolve(!!row)));
            });
          ...
          PRIMARY KEY (cardId, variantKey)
                ...
                await run(`INSERT OR IGNORE INTO graded_refresh_queue_v36 (
          ...
          FROM graded_refresh_queue`);
            ...
            await run('CREATE INDEX IF NOT EXISTS idx_graded_prices_card_variant ON graded_prices(cardId, variantKey)');
            await run('CREATE INDEX IF NOT EXISTS idx_graded_price_history_variant_lookup ON graded_price_history(cardId, variantKey, grader, grade, date)');
            logger_1.logger.info('Migration 36: graded prices/history/queue keyed by variant (finish)');
        },
        down: async () => {
            logger_1.logger.info('Skipping migration 36 rollback (SQLite limitation)');
        },
    },
    {
        id: 37,
        ...
        up: async (db) => {
            ...
            const { setLooksLikeSubsetPrint, numberLooksSecretRare, } = await Promise.resolve().then(() => __importStar(require('../utils/setPrintFamily')));
            const { productIdConflictsWithPrintFamily, resolveProductIdFromOwners, } = await Promise.resolve().then(() => __importStar(require('../utils/productIdGuard')));
            const mappings = await all(`SELECT cardId, cardName, setName, cardNumber, productId, uniqueIdentifier, tcgplayerProductId
         FROM card_mappings`);
            ...
            for (const m of mappings) {
                ...
                const conflicts = m.productId != null &&
                    productIdConflictsWithPrintFamily(m.productId, m.setName, owners);
                const resolved = resolveProductIdFromOwners(m.cardName, m.setName, m.cardNumber, tcgcsvCandidates);
                const needsRemap = resolved != null && resolved !== m.productId;
                const needsPidBackfill = resolved != null && resolved === m.productId && !m.tcgplayerProductId;
                const needsHistoryCleanup = Boolean(m.uniqueIdentifier) &&
                    m.productId != null &&
                    (conflicts || needsRemap);
                if (!needsRemap && !needsPidBackfill && !needsHistoryCleanup)
                    continue;
                if (needsRemap || needsPidBackfill) {
                    const nextProductId = resolved;
                    await run(`UPDATE card_mappings
             SET productId = ?, tcgplayerProductId = ?, updatedAt = datetime('now')
             WHERE cardId = ? AND COALESCE(uniqueIdentifier, '') = COALESCE(?, '')`, [nextProductId, String(nextProductId), m.cardId, m.uniqueIdentifier]);
                    mappingsFixed += 1;
                    if (!m.cardId.startsWith('tcgcsv-')) {
                        await run(`UPDATE catalog_cards
               SET tcgplayerProductId = ?
               WHERE cardId = ?`, [String(nextProductId), m.cardId]);
                        catalogFixed += 1;
                    }
                }
               ...
               WHERE uniqueIdentifier = ?
                 ...
                 AND productId = ?`, [m.uniqueIdentifier, oldProductId], function onDone(err) {
                            var _a;
                            if (err)
                                reject(err);
                            else
           ...
           GROUP BY productId`, [m.uniqueIdentifier]);
                for (const bad of badRows) {
                 ...
                 WHERE uniqueIdentifier = ?
                   AND source IN ('tcgdex', 'tcgdex_ja')
                   AND productId = ?`, [m.uniqueIdentifier, bad.productId], function onDone(err) {
                                var _a;
                                if (err)
                                    reject(err);
                                else
            ...
            logger_1.logger.info('Migration 37: repaired subset productId collisions', {
                mappingsFixed,
                catalogFixed,
                historyDeleted,
            });
        ...
        down: async () => {
            logger_1.logger.info('Skipping migration 37 rollback (SQLite limitation)');
        },
    },
    {
        id: 38,
        ...
        up: async (db) => {
            ...
            const { generateUniqueIdentifier } = await Promise.resolve().then(() => __importStar(require('../services/cardIdentifier')));
            const { numberLooksSecretRare } = await Promise.resolve().then(() => __importStar(require('../utils/setPrintFamily')));
            const mappings = await all(`SELECT cardId, cardName, setId, cardNumber, variantKey, uniqueIdentifier, language, matchName
         FROM card_mappings
         WHERE uniqueIdentifier IS NOT NULL AND cardNumber IS NOT NULL`);
            ...
            for (const m of mappings) {
                if (!numberLooksSecretRare(m.cardNumber) || !m.uniqueIdentifier)
                    continue;
                const correctUid = generateUniqueIdentifier(m.setId, m.cardNumber || undefined, m.cardName, m.variantKey || 'normal', {
                    language: (m.language || 'en'),
                    matchName: m.matchName || undefined,
                });
                if (correctUid === m.uniqueIdentifier)
                    continue;
           ...
           WHERE cardId = ? AND COALESCE(uniqueIdentifier, '') = ?`, [correctUid, m.cardId, oldUid]);
                mappingsFixed += 1;
            }
            logger_1.logger.info('Migration 38: repaired letter-prefix uniqueIdentifiers', {
                mappingsFixed,
                historyMoved,
                historyDropped,
            });
        ...
        down: async () => {
            logger_1.logger.info('Skipping migration 38 rollback (SQLite limitation)');
        },
    },
    {
        id: 39,
        ...
        up: async (db) => {
            ...
            created_at TEXT NOT NULL DEFAULT (datetime('now'))
                    ...
                    if (err)
                        ...
                        db.run('CREATE INDEX IF NOT EXISTS idx_grading_feedback_card ON grading_feedback(card_id)', (e2) => (e2 ? reject(e2) : resolve()));
                    }
                });
            });
        },
        ...
        up: async (db) => {
            ...
            updated_at TEXT NOT NULL DEFAULT (datetime('now'))
                    ...
                    if (err)
                        ...
                        db.run('CREATE INDEX IF NOT EXISTS idx_trades_user ON trades(user_id, updated_at DESC)', (e2) => {
                            if (e2)
                                reject(e2);
                            else {
                                db.run('CREATE INDEX IF NOT EXISTS idx_trades_share ON trades(share_token)', (e3) => (e3 ? reject(e3) : resolve()));
                            }
                        });
                    }
                });
    ...
    },
    {
        ...
        up: async (db) => {
            return new Promise((resolve, reject) => {
                db.run(`CREATE TABLE IF NOT EXISTS saved_ebay_deals (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER NOT NULL,
            game TEXT NOT NULL,
            ...
            UNIQUE(user_id, ebay_listing_id)
                    ...
                    if (err)
                        ...
                        db.run('CREATE INDEX IF NOT EXISTS idx_saved_ebay_deals_user ON saved_ebay_deals(user_id, status, updated_at DESC)', (e2) => (e2 ? reject(e2) : resolve()));
                    }
                });
            });
        },
