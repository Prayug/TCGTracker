...
exports.ensureDataQualityTable = ensureDataQualityTable;
exports.runDataQualityChecks = runDataQualityChecks;
exports.getLatestDataQualityChecks = getLatestDataQualityChecks;
...
async function ensureDataQualityTable() {
    await run(`
...
async function runDataQualityChecks() {
    ...
    const latestRun = await get('SELECT MAX(id) AS id FROM prediction_runs');
    if (latestRun === null || latestRun === void 0 ? void 0 : latestRun.id) {
        const total = await count('SELECT COUNT(*) AS n FROM card_predictions WHERE run_id = ?', [latestRun.id]);
        const nullUid = await count(`SELECT COUNT(*) AS n FROM card_predictions
       WHERE run_id = ? AND (unique_identifier IS NULL OR unique_identifier = '')`, [latestRun.id]);
    ...
    const orphanMappings = await count(`SELECT COUNT(*) AS n FROM card_mappings cm
    ...
    const calDistinct = await count(`SELECT COUNT(*) AS n FROM (
    ...
    const uqCal = await get(`SELECT name FROM sqlite_master
    ...
    const signalCol = await get(`SELECT name FROM pragma_table_info('card_predictions') WHERE name = 'signal_score'`);
    checks.push({
    ...
    const days = (_a = span === null || span === void 0 ? void 0 : span.days) !== null && _a !== void 0 ? _a : 0;
    checks.push({
    ...
    const premature = await count(`SELECT COUNT(*) AS n
    ...
    for (const check of checks) {
        await run(`INSERT INTO data_quality_checks
    ...
    logger_1.logger.info('Data quality checks completed', {
...
async function getLatestDataQualityChecks(limit = 50) {
    await ensureDataQualityTable();
    const latest = await get(`SELECT checked_at FROM data_quality_checks ORDER BY checked_at DESC LIMIT 1`);
    if (!(latest === null || latest === void 0 ? void 0 : latest.checked_at)) {
        return { checks: [], runAt: null, passed: 0, warned: 0, failed: 0 };
    }
    const checks = await new Promise((resolve, reject) => {
        (0, database_1.getDb)().all(`SELECT check_name AS checkName, severity, status, metric_value AS metricValue,
              threshold, details_json AS detailsJson
       FROM data_quality_checks
       WHERE checked_at = ?
       ORDER BY CASE status WHEN 'fail' THEN 0 WHEN 'warn' THEN 1 ELSE 2 END
       LIMIT ?`, [latest.checked_at, limit], (err, rows) => {
            if (err)
                return reject(err);
            resolve((rows || []).map((r) => ({
