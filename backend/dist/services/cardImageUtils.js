...
const buildDeterministicImageUrls = async (setId, cardNumber, setName) => {
...
const hasImageMetadataColumns = async () => {
    if (imageColumnCache &&
        Date.now() - imageColumnCache.checkedAt < IMAGE_COLUMN_CACHE_TTL) {
        return imageColumnCache.hasColumns;
    }
    const { getDb } = await Promise.resolve().then(() => __importStar(require('../db/database')));
    const db = getDb();
...
const getImageColumnSelectFragment = async () => {
    return (await (0, exports.hasImageMetadataColumns)()) ? IMAGE_COLUMN_FRAGMENT : '';
};
