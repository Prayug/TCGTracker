...
exports.listingIsNonEnglish = listingIsNonEnglish;
exports.detectListingRiskFlags = detectListingRiskFlags;
exports.parseEbayListingTitle = parseEbayListingTitle;
exports.formatGradeLabel = formatGradeLabel;
...
const JP_SIGNAL_RE = /\b(japanese|japan|jap|jpn|jp|nihongo|from\s*japan|ships?\s*from\s*japan|japan\s*(?:ver(?:sion)?|print|copy|edition)|jp(?:n)?\s*(?:ver(?:sion)?|print|copy|edition)|eevee\s*heroes|moonbreon|terastal\s*festival|sv8a|s6a|s8b|s12a|sv2a)\b|ワンピース|ワンピ(?:ース)?カード?|【|\djp\b/i;
const OTHER_LANG_RE = /\b(korean|\bkor\b|hangul|chinese|mandarin|cantonese|simplified\s*chinese|traditional\s*chinese)\b/i;
const ASIAN_ENGLISH_RE = /\basian\s*(english|eng)\b|\bae\s*(ver(?:sion)?|print|eng(?:lish)?)\b|\benglish\s*asian\b/i;
...
function normalizeOnePieceNumber(raw) {
...
function parsePokemonNumber(title, grade) {
    const fraction = title.match(FRACTION_NUMBER_RE);
    if (fraction) {
        const raw = `${fraction[1]}/${fraction[2]}`;
        return { raw, collector: (0, priceChartingClient_1.normalizeCardNumber)(fraction[1]) };
    }
    const hashed = title.match(HASH_NUMBER_RE);
    if (hashed) {
        return { raw: hashed[1], collector: (0, priceChartingClient_1.normalizeCardNumber)(hashed[1]) };
    }
    const skip = new Set();
    if (grade)
        skip.add((0, priceChartingClient_1.normalizeCardNumber)(grade.grade.split(' ')[0]));
    const standalone = title.matchAll(/\b(\d{3,4})\b/g);
    for (const match of standalone) {
        const collector = (0, priceChartingClient_1.normalizeCardNumber)(match[1]);
        if (!collector || skip.has(collector))
            continue;
        return { raw: match[1], collector };
    }
    return null;
}
function parseLanguage(title, description, itemCountry) {
    const hay = `${title} ${description || ''}`;
    const nonLatin = (0, scriptDetection_1.queryContainsNonEnglishScript)(hay);
    ...
    if ((jp || other) && !en)
        return asianEnglish || (other && !jp) ? 'other' : 'ja';
    if (en && !jp && !other)
        return 'en';
    if ((jp || other) && en)
        return asianEnglish || (other && !jp) ? 'other' : 'ja';
...
function listingIsNonEnglish(title, description, options) {
    const lang = parseLanguage(title, description, options === null || options === void 0 ? void 0 : options.itemCountry);
    return lang === 'ja' || lang === 'other';
}
function detectListingRiskFlags(title, description) {
    const hay = `${title} ${description || ''}`;
    const flags = [];
    const seen = new Set();
...
function parseEbayListingTitle(title, description, options) {
    var _a, _b;
    ...
    return {
        cardNumber: (_a = pokemonNumber === null || pokemonNumber === void 0 ? void 0 : pokemonNumber.raw) !== null && _a !== void 0 ? _a : onePieceNumber,
        collectorNumber: (_b = pokemonNumber === null || pokemonNumber === void 0 ? void 0 : pokemonNumber.collector) !== null && _b !== void 0 ? _b : null,
        onePieceNumber,
        language: parseLanguage(title, description, options === null || options === void 0 ? void 0 : options.itemCountry),
        grade,
        isGraded: grade != null,
        finishFamily: (0, priceChartingClient_1.detectPcFinishFamily)(title, ''),
        riskFlags: detectListingRiskFlags(title, description),
        haystack,
    };
}
function formatGradeLabel(grade, isGraded) {
    if (!isGraded || !grade)
        return 'Raw';
