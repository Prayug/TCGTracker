...
describe('blendSlabMarketMark', () => {
    ...
    it('uses sold guide when there are no asks', () => {
        const mark = (0, slabMarketMark_1.blendSlabMarketMark)({
            soldGuide: 500,
...
describe('listing title filter', () => {
    ...
    it('accepts a PSA 10 BIN of the same card', () => {
        expect((0, ebayBrowseClient_1.isPsa10ActiveListingTitle)('2019 Pokemon Hidden Fates Charizard GX #9/68 PSA 10 GEM MINT', card)).toBe(true);
    });
    it('rejects lots, proxies, and the wrong number', () => {
        expect((0, ebayBrowseClient_1.isPsa10ActiveListingTitle)('Charizard GX Hidden Fates PSA 10 lot of 3', card)).toBe(false);
        expect((0, ebayBrowseClient_1.isPsa10ActiveListingTitle)('Charizard GX #9 Hidden Fates PSA 10 proxy', card)).toBe(false);
        expect((0, ebayBrowseClient_1.isPsa10ActiveListingTitle)('Charizard GX SV49 Hidden Fates Shiny Vault PSA 10', card)).toBe(false);
    });
    it('rejects a PSA 10 of the same name/number from a different set', () => {
        expect((0, ebayBrowseClient_1.isPsa10ActiveListingTitle)('PSA 10 Charizard GX Burning Shadows #9/147 Holo', card)).toBe(false);
    });
});
describe('summarizeListingPrices', () => {
    it('returns lowest and average of inliers', () => {
        const quote = (0, ebayBrowseClient_1.summarizeListingPrices)([4500, 4600, 4700, 4800, 20000]);
