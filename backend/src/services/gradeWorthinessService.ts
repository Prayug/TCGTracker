 ...
 *
 * PSA Value Bulk / Value / Value Plus / Value Max are paused (as of Jun 2026),
 * so the live floor is Regular (~$79.99). Cheap raw cards with modest PSA 10
...
export interface GradeWorthinessRow {
  ...
  premiumPct: number;
  multiple: number;
  /** Estimated PSA grading fee for declared value ≈ PSA 10 price */
  gradingFee: number;
  gradingTier: string;
  /** raw + grading fee */
  costBasis: number;
  /** PSA 10 − raw − fee */
  netProfit: number;
  /** netProfit / costBasis × 100 */
  netRoiPct: number;
  psa10Pop: number;
  psa9Pop: number | null;
  psaTotal: number;
  gemRatePct: number;
  /** Fee / (PSA10 − raw) — gem rate needed to break even vs selling raw */
  breakEvenGemRatePct: number;
  soldListings: number;
  /** Composite score (not hard-capped at 100 for typical cards) */
  score: number;
  /** 0–100 component for UI bars (after-fee net ROI) */
  upliftScore: number;
  /** 0–100 component for UI bars */
  gemEaseScore: number;
  why: string;
  verified: boolean;
  stale: boolean;
...
export interface GradeWorthinessResult {
  ...
  count: number;
  /** How many PSA 10 rows were considered before filters */
  candidates: number;
  scope: 'all' | 'vault';
...
export const PSA_FEE_CONTEXT: GradeWorthinessFeeContext = {
  grader: 'PSA',
  floorFee: 79.99,
  floorTier: 'Regular',
  note:
 ...
 */
const SORT_MODES = new Set<GradeWorthinessSort>([
  'score',
  'netProfit',
  'netRoi',
  'gemEase',
...
export function parseGradeWorthinessSort(value: unknown): GradeWorthinessSort {
  const s = String(value || 'score').trim();
  return SORT_MODES.has(s as GradeWorthinessSort) ? (s as GradeWorthinessSort) : 'score';
}

function sortWorthinessRows(rows: GradeWorthinessRow[], sort: GradeWorthinessSort): void {
  switch (sort) {
    case 'netProfit':
    ...
    case 'scarce':
      rows.sort(
        (a, b) => a.gemRatePct - b.gemRatePct || a.psa10Pop - b.psa10Pop || b.score - a.score
      );
      break;
    case 'score':
    default:
...
export async function getGradeWorthinessLeaderboard(options?: {
  limit?: number;
  cardIds?: string[];
  eras?: string[];
  setIds?: string[];
  sort?: GradeWorthinessSort;
}): Promise<GradeWorthinessResult> {
  const limit = Math.min(Math.max(options?.limit ?? 40, 1), 200);
  const cardIds = options?.cardIds?.filter(Boolean) ?? [];
  const eras = [...new Set((options?.eras ?? []).map((e) => e.trim()).filter(Boolean))];
  const setIds = [...new Set((options?.setIds ?? []).map((s) => s.trim()).filter(Boolean))];
  const sort = options?.sort ?? 'score';
  const scope: 'all' | 'vault' = cardIds.length > 0 ? 'vault' : 'all';
  ...
  const rows = await all<RawCandidate>(
    `SELECT
       gp.cardId,
  ...
  for (const r of rows) {
    ...
    if (!(rawPrice >= MIN_RAW_PRICE)) continue;
    if (!(psa10Price > rawPrice)) continue;
    if (!(psaTotal >= MIN_PSA_TOTAL) || !(psa10Pop >= 0)) continue;

    const premium = psa10Price - rawPrice;
    const premiumPct = (premium / rawPrice) * 100;
    if (premiumPct < MIN_PREMIUM_PCT) continue;
    ...
    if (netProfit < MIN_NET_PROFIT || netRoiPct < MIN_NET_ROI_PCT) continue;
    ...
    const psa9Pop =
      r.psa9Pop != null && Number.isFinite(Number(r.psa9Pop)) ? Number(r.psa9Pop) : null;
  ...
  if (setIds.length > 0) {
    ...
    filtered = filtered.filter((r) => {
      if (r.setId && setSet.has(r.setId.toLowerCase())) return true;
      // tcgcsv rows sometimes lack setId — allow setName key match
      if (r.setName && setSet.has(`name:${r.setName.toLowerCase()}`)) return true;
      return false;
    });
  }
  ...
  return {
    ...
    scope,
    feeContext: PSA_FEE_CONTEXT,
    facets,
    filters: { eras, setIds },
  };
}

function buildFacets(rows: GradeWorthinessRow[]): GradeWorthinessResult['facets'] {
  const eraCounts = new Map<string, number>();
  const setCounts = new Map<string, GradeWorthinessSetFacet>();
  ...
  const eras: GradeWorthinessEraFacet[] = ERA_GROUPS.map((g) => ({
    id: g.id,
    label: g.label,
  ...
  for (const [id, count] of eraCounts) {
    if (!eras.some((e) => e.id === id)) {
      eras.push({ id, label: getEraLabel(id), count });
    }
  }
...
async function fillMissingImages(rows: GradeWorthinessRow[]): Promise<void> {
  ...
  await Promise.all(
    missing.map(async (row) => {
      ...
      const exactName = candidates.filter((c) => normalizeName(c.cardName) === nameKey);
