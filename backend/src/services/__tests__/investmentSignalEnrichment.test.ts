...
describe('investmentSignalEnrichment', () => {
  it('maps social source to reddit category', () => {
    expect(mapSignalCategory('social', null)).toBe('reddit');
    expect(mapSignalCategory('set_release', 'upcoming_set')).toBe('set_release');
  });
  ...
  it('raises confidence with market data and sources', () => {
    const low = computeSignalConfidence({
    ...
    const high = computeSignalConfidence({
      hasCardMetrics: true,
      hasSetMetrics: false,
      volumeChangePct: 15,
      sourceCount: 6,
  ...
  it('extracts set entity from youtube title', () => {
    ...
    expect(buildEntityLabel(entity.entityType, entity.entityName)).toContain('(Set)');
  });
