...
describe('prediction tracking quality', () => {
  ...
  it('rejects cliffy step-function series like Sharpedo holofoil', () => {
    ...
    const history = dedupePriceHistoryByDate(prices);
    expect(countDistinctPrices(history)).toBeLessThan(5);
    expect(calendarSpanDays(history)).toBeGreaterThan(20);
    expect(
      hasAdequateTrackingHistory(history, countLiveQuotes(prices), {
        minPoints: 14,
