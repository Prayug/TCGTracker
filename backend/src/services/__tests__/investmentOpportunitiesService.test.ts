import {
  ...
  type SeriesPoint,
} from '../investmentOpportunitiesService';

describe('computeChange (mover math)', () => {
  it('computes absolute and percent change', () => {
    expect(computeChange(120, 100)).toEqual({ changeAbs: 20, changePct: 20 });
    expect(computeChange(90, 100)).toEqual({ changeAbs: -10, changePct: -10 });
  });

  it('guards against zero previous price', () => {
    expect(computeChange(50, 0).changePct).toBe(0);
  });
});
...
describe('passesMoverThresholds', () => {
  ...
  it('accepts a $5+ move above min price with enough history', () => {
    expect(passesMoverThresholds(base)).toBe(true);
  });
  ...
  it('rejects series with fewer than 3 history points', () => {
    expect(passesMoverThresholds({ ...base, historyPoints: 2 })).toBe(false);
  });
});

describe('characterToken', () => {
  it('extracts the character from suffixed names', () => {
    expect(characterToken('Charizard ex')).toBe('charizard');
    expect(characterToken('Mega Charizard EX')).toBe('charizard');
    expect(characterToken('Pikachu VMAX')).toBe('pikachu');
    expect(characterToken('Radiant Greninja')).toBe('greninja');
  });

  it('returns null for empty input', () => {
    expect(characterToken(null)).toBeNull();
    expect(characterToken('')).toBeNull();
  });
});

describe('moveCorrelation', () => {
  const series = (prices: number[]): SeriesPoint[] =>
    prices.map((price, i) => ({
  ...
  it('finds strong positive correlation for parallel moves', () => {
    ...
    const result = moveCorrelation(anchor, comp);
...
describe('computeBuyoutScore (thresholds)', () => {
  ...
  it('requires a measured >=30% drop from a meaningful baseline', () => {
    const smallBaseline = computeBuyoutScore({
      spikePct: 30,
      listedCount: 1,
      listedCountPrev: 2,
      velocityRatio: null,
    ...
    const shallowDrop = computeBuyoutScore({
      spikePct: 30,
      listedCount: 10,
      listedCountPrev: 12,
      velocityRatio: null,
    ...
    const realDrain = computeBuyoutScore({
      spikePct: 30,
      listedCount: 4,
      listedCountPrev: 12,
      velocityRatio: null,
...
describe('computeOpportunityScore (weighting)', () => {
  ...
  it('pulls low-confidence predictions toward neutral', () => {
    const confident = computeOpportunityScore({
      predictedReturn90d: 35,
      confidence: 90,
      momentumPct: 0,
      buyoutScore: 0,
    ...
    const shaky = computeOpportunityScore({
      predictedReturn90d: 35,
      confidence: 10,
      momentumPct: 0,
      buyoutScore: 0,
  ...
  it('falls back to momentum-weighted scoring without a prediction', () => {
    const withMomentum = computeOpportunityScore({
