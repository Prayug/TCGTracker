...
export interface ValidationMetrics {
  /** Spearman rank correlation between predicted and actual returns in [-1, 1]. */
  rankIC: number | null;
  /** Mean absolute error of predicted vs actual returns. */
  mae: number | null;
  ...
  /** Fraction of predictions whose error beat a skill-relative threshold. */
  hitRate: number | null;
}
 ...
 */
export function computeRankIC(predicted: number[], actual: number[]): number | null {
  if (predicted.length !== actual.length || predicted.length < 8) return null;
  ...
  if (!Number.isNaN(spearman)) return spearman;
...
export function computeHitRate(
  ...
  for (const s of valid) {
    const actual = s.actual as number;
...
export function computeBias(samples: Sample[]): number | null {
  const valid = samples.filter((s) => s.actual !== null);
  if (valid.length === 0) return null;
  const diffs = valid.map((s) => s.predicted - (s.actual as number));
  return median(diffs);
}

export function computeValidationMetrics(samples: Sample[]): ValidationMetrics {
  const valid = samples.filter((s) => s.actual !== null) as Array<{
    predicted: number;
    actual: number;
  }>;
  if (valid.length === 0) {
  ...
  return {
    rankIC: computeRankIC(
      valid.map((s) => s.predicted),
      valid.map((s) => s.actual)
    ),
