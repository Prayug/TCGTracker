...
import { getRawTopMovers } from '../services/rawTopMovers';
...
router.get('/top-movers', async (req: Request, res: Response) => {
  ...
  try {
    const payload = await getRawTopMovers(days, limit);
    res.setHeader('Cache-Control', `public, max-age=${Math.floor(TOP_MOVERS_TTL_MS / 1000)}`);
    res.json(payload);
  } catch (error) {
    logger.error('Top movers query failed', { error: (error as Error).message });
    res.status(500).json({ error: (error as Error).message });
  }
});
