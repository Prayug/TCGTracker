    ...
    <nav aria-label="Quick actions" className="flex flex-wrap items-center gap-2">
      {ACTIONS.map(({ key, label, hint, href, icon: Icon }) => (
        <Link
          key={key}
          to={href}
          className="group inline-flex cursor-pointer items-center gap-2 rounded-full border border-border-subtle bg-surface-raised/60 py-2 pl-3 pr-4 text-sm transition-colors duration-200 hover:border-accent/40 hover:bg-surface-hover"
        >
          <Icon className="h-3.5 w-3.5 text-foil transition-colors group-hover:text-accent" />
