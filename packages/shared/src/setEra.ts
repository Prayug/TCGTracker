...
/** Newest eras first — same order as ERA_GROUPS, without sortOrder. */
export const ERA_ORDER: Array<{ id: string; label: string }> = ERA_GROUPS.map(({ id, label }) => ({
  id,
  label,
}));
...
export const classifySetEra = (input: { id: string; name: string; series?: string }): string => {
  ...
  if (
    series.includes('mega') ||
    /^me\d|^me-|^me_|^mep$/.test(id) ||
    name.includes('mega evolution')
  ) {
    return 'mega';
  }
  if (
